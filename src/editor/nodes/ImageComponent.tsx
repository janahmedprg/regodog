...
function LazyImage({
  ...
  const calculateDimensions = () => {
    ...
    function isImageSuccess(
      status: ImageStatus
    ): status is { error: false; width: number; height: number } {
      return !status.error;
    }
    ...
    if (isImageSuccess(status)) {
      naturalWidth = status.width; // TypeScript now knows status has width/height
      naturalHeight = status.height;
    }
